...
(function() {
  ...
  console.log('Inventor\'s names: ', inventorsNames);

  // Array.prototype.sort()
  // 3. Sort the inventors by birthdate, oldest to youngest
  const sortedInventors = inventors.sort((inventorA, inventorB) => {
  ...
  const years = inventors.reduce((previous, current, i, arr ) => {
    return previous + (arr[i].passed - arr[i].year);
  }, 0);
  console.log(years);

  // 5. Sort the inventors by years lived
  const oldestInventors = inventors.sort((inventorA, inventorB) => {
    const yearsA = inventorA.passed - inventorA.passed;
    const yearsB = inventorB.passed - inventorB.passed;
  ...
  const toFilter = links.map(link => link.textContent);
  const containDE = toFilter.filter(link => link.includes('de'));
